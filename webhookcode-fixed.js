...
/**
 * Find QR transaction by policy number
 * Searches the nic_qr_transactions table for matching records
 */
async function findQRTransaction(policyNumber) {
  try {
    ...
    if (paymentStatusCode === 'ACSP') {
      ...
      if (updateResult.success) {
        ...
        if (qrTransactionProcessed) {
          ...
          console.log(`   Customer Confirmation: ${customerNotificationSent ? '✅ Sent' : '❌ Failed'}`);
...
app.listen(PORT, () => {
  ...
  console.log('   ✅ Customer Payment Confirmations');
